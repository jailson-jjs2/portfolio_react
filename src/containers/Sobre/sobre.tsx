import Titulo from '../../components/Titulo/titulo'
import Paragrafo from '../../components/Paragrafo/paragrafo'
import { GithubSecao } from './styles'

const Sobre = () => (
  <section>
    <Titulo fontSize={16}>Sobre</Titulo>
    <Paragrafo tipo="principal">
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed temporibus
      quidem, ducimus mollitia consequatur laboriosam doloremque delectus maxime
      veniam praesentium inventore nulla quis cupiditate doloribus, harum
      pariatur amet modi libero!
    </Paragrafo>
    <GithubSecao>
      <img src="https://github-readme-stats.vercel.app/api?username=Jailson-jjs2&show_icons=true&theme=dracula&include_all_commits=true&count_private=true" />
      <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Jailson-jjs2&layout=compact&langs_count=7&theme=dracula" />
    </GithubSecao>
  </section>
)

export default Sobre
